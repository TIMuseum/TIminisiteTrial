...
//CAMERA VARIABLES
const fov = 75;
const camStart = {x:0, y:1500, z:0}; 
const camMain = {x:0, y:250, z:0};; 
...
function clickEgg(clicked, offset, index){
  ...
  let  coords = { x: camera.position.x, y: camera.position.y, z: camera.position.z };
    ...
    .onUpdate(() =>{
      camera.position.set(coords.x+offset.x, coords.y+offset.y, coords.z+ offset.z)
      
    })
    .start();
    console.log("cam x " + camera.position.x +" cam y " + camera.position.y +" cam y " +camera.position.z )
...
function exitEgg(){
  ...
  console.log("camera x " + coords.x + " camera y " + coords.y + " camera z " + coords.z); 
  var tween = new TWEEN.Tween(coords)
  .to({ x: camMain.x, y: camMain.y, z:camMain.z}, 1500)
  .easing(TWEEN.Easing.Quadratic.Out)
  .onUpdate(() =>{
